  ...
  if(mode==='santri'){
    ...
    const rows=paginate(data,'rekap').map((d,i)=>{
      ...
      return `<tr>
        ...
        <td><div style="display:flex;gap:4px">${d.tunggak>0?`<button class="btn btn-p btn-xs mutating-only" onclick="bukaMoBayarMulti('${d.id}')">${svgIcon('cash',12)}</button>`:''}<button class="btn btn-b btn-xs" onclick="openDetailModal('${d.id}')">${svgIcon('document',12)}</button>${d.tunggak>0?(d.no_wa?`<button class="btn btn-wa btn-xs mutating-only" onclick="kirimWASantriRekap('${d.id}')">${svgIcon('smartphone',12)}</button>`:`<span style="display:inline-flex;cursor:default;opacity:.5" title="No WA belum diinput">${svgIcon('smartphone',14)}</span>`):''}</div></td>
      </tr>`;
    }).join('');
